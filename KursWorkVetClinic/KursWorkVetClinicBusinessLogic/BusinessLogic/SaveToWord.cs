        ...
        public static void CreateDoc(WordExelInfo info)
        {
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
            {
                ...
                if (info.Services != null)
                {
                    ...
                    {
                        var result = string.Join(", ", service.ToArray());
                        docBody.AppendChild(CreateParagraph(new WordParagraph
                        {
                            Texts = new List<(string, WordTextProperties)> { (service.AnimalName + ": ", new WordTextProperties {Bold = true, Size = "24", }),
                            (result, new WordTextProperties { Size = "24", })},
                            TextProperties = new WordTextProperties
                            {
                                Size = "24",
